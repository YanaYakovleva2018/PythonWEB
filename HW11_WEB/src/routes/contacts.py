...
async def get_contact(skip: int = 0, limit: int = Query(default=10),
...
async def get_birthdays(skip: int = 0, limit: int = Query(default=10), db: Session = Depends(get_db)):
    contacts = await repository_contacts.get_birthdays(skip, limit, db)
    if not contacts:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contacts not found")
    return contacts
...
async def get_contact_id(contact_id: int, db: Session = Depends(get_db)):
